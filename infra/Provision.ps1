...
if (-not $SkipInfra) {
    ...
    try {
        ...
        if ($existing -notcontains 'Jwt--SigningKey') {
        ...
        if ($existing -notcontains 'ConnectionStrings--WidgetWorks') {
            Write-Step 'ConnectionStrings--WidgetWorks'
            Write-Host '  Paste the Neon connection string (input hidden).'
    ...
    Invoke-Az webapp config appsettings set --name $AppName --resource-group $ResourceGroup --settings `
...
Invoke-Az webapp config appsettings set --name $AppName --resource-group $ResourceGroup --settings `
    "Cors__AllowedOrigins=$SwaUrl" "App__BaseUrl=$SwaUrl" --output none | Out-Null
Write-Ok 'Set'
